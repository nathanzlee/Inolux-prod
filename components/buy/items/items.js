        ...
        <div className="overflow-hidden shadow ring-1 ring-black ring-opacity-5 sm:rounded-lg mt-5 max-h-[600px] overflow-y-auto">
            <table className="min-w-full divide-y divide-gray-300">
                ...
                <tbody className="divide-y divide-gray-200 bg-white">
                    {data.map((item) => {
                        const {_id, name, documentNo, unitMeasurement, unitCost} = item
                        return (
                            <Row key={_id} id={_id} partNo={name} unit={unitMeasurement} code={documentNo} cost={unitCost} edit={edit}/>
                        )
                    })}
                </tbody>
            </table>
